...
export function ChatbotPage() {
  ...
  const demoQa = useMemo(() => ([
    { q: t('chatbot.prompt1'), a: t('chatbot.answer1') },
    { q: t('chatbot.prompt2'), a: t('chatbot.answer2') },
    { q: t('chatbot.prompt3'), a: t('chatbot.answer3') },
    { q: t('chatbot.prompt4'), a: t('chatbot.answer4') },
    { q: t('chatbot.prompt5'), a: t('chatbot.answer5') },
    { q: t('chatbot.prompt6'), a: t('chatbot.answer6') },
  ]), [t])
  ...
  const handleSend = async (forcedText) => {
    ...
    try {
      ...
      const matched = demoQa.find((item) => item.q.trim().toLowerCase() === normalized)
      const reply = matched?.a || t('chatbot.noResponse')
      addMessage('assistant', reply)
    } catch (error) {
      const detail = error?.response?.data?.detail || t('chatbot.serviceUnavailable')
      setErrorText(detail)
      addMessage('assistant', t('chatbot.tempUnavailable'))
  ...
  // Keep welcome message and timestamp format in sync with active app language.
  useEffect(() => {
    if (messages.length === 1 && messages[0]?.id === 'welcome') {
      setMessages(initialMessages)
    }
  }, [i18n.language, initialMessages, messages])
    ...
    <div className="chatbot-root bg-background font-body text-on-surface selection:bg-secondary-container">
      ...
      <main className="min-h-screen px-4 pb-28 pt-20 lg:ml-64 lg:px-8">
        ...
        <section className="mb-8 mt-4 text-center">
          <h1 className="font-headline mb-2 text-3xl font-extrabold text-primary">{t('chatbot.title')}</h1>
          <p className="font-medium text-on-surface-variant">{t('chatbot.subtitle')}</p>
        </section>
          ...
          <div className="mb-6 flex items-center justify-between px-2">
            <div className="flex gap-2">
              <span className="flex items-center gap-1 rounded-full bg-secondary-container px-3 py-1 text-xs font-bold text-on-secondary-container">
                ...
                {isLoading ? t('chatbot.thinking') : t('chatbot.liveAdvice')}
              </span>
            </div>
            <div className="flex gap-3">
              <button className="rounded-full p-2 text-on-surface-variant transition-colors hover:bg-surface-container-high hover:text-primary" title={t('chatbot.clearChat')} type="button" onClick={handleClear}>
                ...
                return (
                  <div key={message.id} className="flex max-w-[85%] gap-4">
                    ...
                    <div className="asymmetric-card border border-outline-variant/10 bg-surface-container-lowest p-5 shadow-sm">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-widest text-stone-400">{t('chatbot.advisorLabel')}</span>
                    </div>
                  </div>
                )
              }
              ...
              return (
                <div key={message.id} className="ml-auto flex max-w-[85%] flex-row-reverse gap-4">
                  ...
                  <div className="rounded-2xl rounded-tr-none bg-primary-container p-5 text-on-primary-container shadow-md">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-widest opacity-70">{t('chatbot.youLabel')}</span>
                  </div>
                </div>
              )
            })}
            ...
            {isLoading && (
              <div className="flex max-w-[85%] items-end gap-4">
                ...
                <div className="flex items-center gap-2 rounded-2xl rounded-bl-none bg-surface-container-lowest px-6 py-4 shadow-sm">
                  ...
                  <span className="ml-2 text-xs font-semibold italic text-primary/60">{t('chatbot.analyzing')}</span>
                </div>
              </div>
            )}
            ...
            <div className="mt-4 flex items-center justify-between px-2">
              ...
              <div className="flex items-center gap-3">
                <button className="flex items-center gap-1 text-xs font-bold text-on-surface-variant transition-colors hover:text-primary" type="button" onClick={handleCopyLast}>
                  ...
                  {copied ? t('chatbot.copied') : t('chatbot.copyLast')}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="mt-8 flex items-center gap-4 rounded-2xl border border-secondary-fixed bg-secondary-fixed/30 p-4">
          ...
          <div>
            ...
            <p className="text-xs leading-snug text-on-secondary-fixed-variant opacity-80">{t('chatbot.trustText')}</p>
          </div>
        </div>
        </div>
      </main>
